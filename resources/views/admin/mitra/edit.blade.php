...
<span class="inline-flex items-center gap-2">
  ...
  <span>Ubah Mitra Industri</span>
</span>
@endsection
@section('subtitle', 'Ubah data dan logo mitra industri')
...
<div class="grid grid-cols-1 xl:grid-cols-3 gap-6 items-start">
  ...
  <div class="xl:col-span-2">
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
      <form method="POST" action="{{ route('admin.mitra.update', $mitra) }}" enctype="multipart/form-data" class="p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="md:col-span-2">
            ...
            <input type="text" id="nama" name="nama" value="{{ old('nama', $mitra->nama) }}" required
          ...
          <div class="md:col-span-2">
            ...
            <input type="text" id="link" name="link" value="{{ old('link', $mitra->link) }}"
          ...
          <div>
            ...
            <input type="number" id="urutan" name="urutan" value="{{ old('urutan', $mitra->urutan) }}" required
          ...
          <div class="flex items-start pt-7">
            <div class="flex items-center h-5">
              <input id="aktif" name="aktif" type="checkbox" value="1" {{ old('aktif', $mitra->aktif) ? 'checked' : '' }}
        ...
        <div class="border-t border-slate-100 pt-6">
          ...
          <div class="flex items-center gap-4">
            <div id="preview-container" class="w-28 h-14 rounded border border-slate-200 bg-slate-50 flex items-center justify-center p-1 overflow-hidden flex-shrink-0">
              <img id="image-preview" src="{{ $mitra->logo_src }}" alt="Preview" class="max-w-full max-h-full object-contain">
            </div>
            <label class="cursor-pointer flex items-center gap-2 px-4 py-3 rounded-xl border-2 border-dashed border-slate-200 hover:border-primary hover:bg-orange-50 transition-all text-sm text-slate-500 hover:text-primary">
              ...
              <span id="file-label">Ganti Logo Mitra (Maks. 2MB)</span>
        ...
        <div>
          ...
          <input type="url" id="logo_url" name="logo_url" value="{{ old('logo_url', $mitra->logo_url) }}"
            class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none transition text-sm"
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden sticky top-6">
      ...
      @else
        <ul class="divide-y divide-slate-50 max-h-[480px] overflow-y-auto">
          ...
          <li class="flex items-center gap-3 px-5 py-3 transition-colors group
            {{ $item->id === $mitra->id ? 'bg-primary/5 border-l-4 border-primary' : 'hover:bg-slate-50' }}">
            {{-- Logo --}}
            ...
            <div class="flex-1 min-w-0">
              <p class="text-sm font-medium truncate {{ $item->id === $mitra->id ? 'text-primary' : 'text-slate-700' }}">
                {{ $item->nama }}
                @if($item->id === $mitra->id)
                  <span class="text-[10px] font-semibold text-primary/70">(sedang diedit)</span>
                @endif
              </p>
            ...
            {{-- Actions (tersembunyi sampai hover, kecuali yang sedang diedit) --}}
            @if($item->id !== $mitra->id)
