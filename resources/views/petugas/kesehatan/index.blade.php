...
@section('title', 'UKS & Pemeriksaan Kesehatan')
@section('subtitle', 'Cek tinggi/berat badan, tensi darah, buta warna, tato/tindik, dan riwayat penyakit calon siswa')
...
<div class="max-w-6xl mx-auto space-y-6">
  ...
  <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 flex items-center justify-between">
      <div>
        <span class="text-xxs font-bold text-slate-400 uppercase block tracking-wider">Belum Pemeriksaan</span>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 flex items-center justify-between">
      <div>
        <span class="text-xxs font-bold text-slate-400 uppercase block tracking-wider">Sudah Pemeriksaan</span>
  ...
  <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
    <div class="flex items-center gap-2">
      <a href="{{ route('petugas.kesehatan.dashboard') }}" 
      ...
      <a href="{{ route('petugas.kesehatan.dashboard') }}?filter=belum" 
      ...
      <a href="{{ route('petugas.kesehatan.dashboard') }}?filter=sudah" 
         class="px-4 py-2 text-xs font-semibold rounded-xl transition {{ request('filter') === 'sudah' ? 'bg-blue-600 text-white shadow-sm' : 'bg-slate-50 hover:bg-slate-100 text-slate-600' }}">
        Sudah Tes
      </a>
    </div>

    <form method="GET" action="{{ route('petugas.kesehatan.dashboard') }}" class="flex items-center gap-2">
      @if(request('filter'))
      ...
      <table class="w-full text-left border-collapse text-slate-700">
        <thead>
          <tr class="bg-slate-50 border-b border-slate-100 text-slate-500 text-xxs uppercase tracking-wider font-bold">
            ...
            <th class="py-3 px-5">Tinggi / Berat / Tensi</th>
          ...
          <tr class="hover:bg-slate-50/50 transition">
            ...
            <td class="py-4 px-5 text-center">
              <a href="{{ route('petugas.kesehatan.show', $p->id) }}" 
                ...
                <i class="fas fa-heartbeat"></i> Periksa Kesehatan
              </a>
            </td>
          </tr>
          @empty
