...
@endsection
@section('subtitle', 'Ubah data dan konfigurasi slide carousel')

@section('content')
<div class="max-w-3xl">
  <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
    <form method="POST" action="{{ route('admin.hero.update', $hero) }}" enctype="multipart/form-data" class="p-6 space-y-6">
      @csrf
      @method('PUT')
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="md:col-span-2">
          ...
          <input type="text" id="judul" name="judul" value="{{ old('judul', $hero->judul) }}" required
        ...
        <div class="md:col-span-2">
          ...
          <textarea id="deskripsi" name="deskripsi" rows="3"
            ...
            placeholder="Siswa SMK Muhammadiyah 1 Bantul mengasah keterampilan di bengkel otomotif industri...">{{ old('deskripsi', $hero->deskripsi) }}</textarea>
        ...
        <div>
          ...
          <input type="number" id="urutan" name="urutan" value="{{ old('urutan', $hero->urutan) }}" required
        ...
        <div class="flex items-start pt-7">
          <div class="flex items-center h-5">
            <input id="aktif" name="aktif" type="checkbox" value="1" {{ old('aktif', $hero->aktif) ? 'checked' : '' }}
        ...
        <div>
          ...
          <input type="text" id="label_tombol" name="label_tombol" value="{{ old('label_tombol', $hero->label_tombol) }}"
            class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none transition text-sm"
        ...
        <div>
          ...
          <input type="text" id="link_tombol" name="link_tombol" value="{{ old('link_tombol', $hero->link_tombol) }}"
            class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none transition text-sm"
      ...
      <div class="border-t border-slate-100 pt-6">
        <label class="block text-sm font-medium text-slate-700 mb-1.5">Unggah Gambar Slide Baru (Akan menggantikan gambar lama)</label>
        <div class="flex items-center gap-4">
          <div id="preview-container" class="w-32 h-20 rounded-xl overflow-hidden border border-slate-200">
            <img id="image-preview" src="{{ $hero->gambar_src }}" alt="Preview" class="w-full h-full object-cover">
          </div>
          <label class="cursor-pointer flex items-center gap-2 px-4 py-3 rounded-xl border-2 border-dashed border-slate-200 hover:border-primary hover:bg-orange-50 transition-all text-sm text-slate-500 hover:text-primary">
            ...
            <span id="file-label">Ganti File Gambar</span>
      ...
      <div>
        ...
        <input type="url" id="gambar_url" name="gambar_url" value="{{ old('gambar_url', $hero->gambar_url) }}"
          class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none transition text-sm"
