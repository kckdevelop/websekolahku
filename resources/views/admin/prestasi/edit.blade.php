...
@endsection
@section('subtitle', 'Perbarui data prestasi dan penghargaan')
...
<div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
  ...
  <div class="xl:col-span-2">
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
      ...
      <form method="POST" action="{{ route('admin.prestasi.update', $prestasi) }}" enctype="multipart/form-data" class="p-6 space-y-5">
        @csrf @method('PUT')
        <div>
          <label class="block text-sm font-medium text-slate-700 mb-1.5">Judul Prestasi <span class="text-red-500">*</span></label>
          <input type="text" name="judul" value="{{ old('judul', $prestasi->judul) }}" required class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none transition text-sm">
        </div>
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-1.5">Kategori <span class="text-red-500">*</span></label>
            <input type="text" name="kategori" value="{{ old('kategori', $prestasi->kategori) }}" required class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none transition text-sm">
          </div>
          <div>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" name="peraih" value="{{ old('peraih', $prestasi->peraih) }}" required class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none transition text-sm">
          </div>
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-1.5">Tanggal <span class="text-red-500">*</span></label>
            <input type="date" name="tanggal" value="{{ old('tanggal', $prestasi->tanggal->format('Y-m-d')) }}" required class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none transition text-sm">
          </div>
        </div>
        <div>
          <label class="block text-sm font-medium text-slate-700 mb-1.5">Deskripsi <span class="text-red-500">*</span></label>
          <textarea name="deskripsi" required rows="4" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none transition text-sm resize-none">{{ old('deskripsi', $prestasi->deskripsi) }}</textarea>
        </div>
        <div>
          ...
          <div id="drop-zone" class="border-2 border-dashed border-slate-200 hover:border-primary/50 rounded-2xl p-6 transition-all cursor-pointer text-center bg-slate-50/50 hover:bg-slate-50 relative">
            ...
            <div class="space-y-2 pointer-events-none">
              ...
              <p class="text-sm font-medium text-slate-700">
                <span class="text-primary font-semibold">Klik untuk ubah foto</span> atau seret dan lepas gambar ke sini
              </p>
              ...
              <!-- Preview Container -->
              <div id="preview-container" class="{{ $prestasi->foto ? '' : 'hidden' }} mt-3">
                <img id="image-preview" src="{{ $prestasi->foto ? asset('storage/' . $prestasi->foto) : '' }}" class="mx-auto w-32 h-24 object-cover rounded-lg border border-slate-200 shadow-sm">
              </div>
            </div>
          </div>
        ...
        <div class="flex gap-3 pt-2">
          ...
          <a href="{{ route('admin.prestasi.index') }}" class="inline-flex items-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold px-6 py-3 rounded-xl">Batal</a>
        </div>
      </form>
    </div>
  </div>
  ...
  <div class="xl:col-span-1">
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden sticky top-6">
      ...
      <div class="divide-y divide-slate-50">
        ...
        <div class="px-5 py-3 flex items-start gap-3 hover:bg-slate-50 transition-colors group {{ $item->id === $prestasi->id ? 'bg-orange-50 border-l-2 border-primary' : '' }}">
          @if($item->foto)
