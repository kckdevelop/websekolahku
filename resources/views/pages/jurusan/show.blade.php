...
@section('content')
  ...
  <section class="relative h-[40vh] overflow-hidden">
    @auth
      <div class="absolute top-6 right-6 z-[999]">
        <a href="{{ route('admin.jurusan.edit', $content) }}"
           class="bg-primary hover:bg-secondary text-white text-sm font-semibold px-5 py-3 rounded-full shadow-lg flex items-center gap-2 transition transform hover:scale-105">
  ...
  {{-- Tab Jurusan Lainnya (Dinamis dari DB) --}}
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex overflow-x-auto gap-1 py-2 scrollbar-hide">
        @foreach($allJurusan as $j)
          <a href="/jurusan/{{ $j->slug }}"
             class="jurusan-tab {{ $j->slug === $content->slug ? 'active' : 'text-slate-600 dark:text-slate-300 hover:bg-orange-50 dark:hover:bg-slate-700' }} whitespace-nowrap px-4 py-2 rounded-lg text-sm font-medium transition">
            <i class="{{ $j->icon ?? 'fas fa-graduation-cap' }} mr-1"></i>
            {{ $j->nama_jurusan }}
          </a>
        @endforeach
      </div>
