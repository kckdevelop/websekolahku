...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
  ...
  <div class="p-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
    <div class="flex items-center gap-2">
      <h2 class="font-bold text-slate-800 text-sm flex items-center gap-2">
        <i class="fas fa-heartbeat text-rose-500"></i> Data Pemeriksaan Kesehatan
      </h2>
    </div>
    <div class="flex gap-2 flex-wrap items-center">
      ...
      <select id="ajax-filter" class="text-sm border border-slate-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-300 bg-white text-slate-600">
        ...
        <option value="belum" {{ request('filter') === 'belum' ? 'selected' : '' }}>Belum Diperiksa</option>
        <option value="sudah" {{ request('filter') === 'sudah' ? 'selected' : '' }}>Sudah Diperiksa</option>
      </select>
      {{-- Search --}}
      <div class="relative">
  ...
  <div id="table-wrapper">
    @include('petugas.kesehatan.table')
  </div>
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function fetchTable(url = "{{ route('petugas.kesehatan.dashboard') }}") {
